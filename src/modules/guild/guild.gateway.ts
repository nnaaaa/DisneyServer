...
export class GuildGateway {
    ...
    async joinGuild(@MessageBody() guildId: string, @AuthWSUser() authUser: UserEntity) {
        try {
            ...
            // emit to guild
            this.server.emit(`${GuildSocketEmit.USER_JOIN}/${guildId}`, newMember)
            ...
            this.channelGateway.guildMemberNotify(ChannelSocketEmit.USER_JOIN, newMember)
    ...
    async leaveGuild(@MessageBody() guildId: string, @AuthWSUser() authUser: UserEntity) {
        try {
            ...
            // emit to guild
            this.server.emit(
                `${GuildSocketEmit.USER_LEAVE}/${guildId}`,
                memberLeaveGuild
            )

            // emit to all channel which user have joined
            this.channelGateway.guildMemberNotify(ChannelSocketEmit.USER_LEAVE, memberLeaveGuild)
        } catch (e) {
            this.logger.error(e)
            throw new WsException(e)
        }
    ...
    async updateMember(
        ...
        try {
            ...
            // emit to guild
            this.server.emit(
                `${GuildSocketEmit.USER_UPDATE}/${member.guild.guildId}`,
                member
            )

            // emit to all channel which user have joined
            this.channelGateway.guildMemberNotify(ChannelSocketEmit.USER_UPDATE, member)
        } catch (e) {
            this.logger.error(e)
        }
    }
