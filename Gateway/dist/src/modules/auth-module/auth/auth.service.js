...
let AuthService = class AuthService {
    ...
    async verifyAuthUserToChangePassword(newPassordWithSMSDto) {
        ...
        if (!digitCode)
            throw new common_1.ForbiddenException();
        const user = await this.userService.findOne({ account }, { userId: true, password: true, changePwdVerfiyCode: true });
        if (!user)
            throw new common_1.NotFoundException();
    ...
    async verifyAuthUserToRegister({ account, digitCode }) {
        const user = await this.userService.findOne({ account }, { userId: true, registerVerifyCode: true });
        if (!user)
            throw new common_1.NotFoundException();
        if (!user.registerVerifyCode)
            throw new common_1.ForbiddenException();
    ...
    async changeUserPassword(newPassordWithComparation) {
        ...
        const user = await this.userService.findOne({ account }, { userId: true, password: true, registerVerifyCode: true });
        if (!user)
            throw new common_1.NotFoundException();
        if (user.registerVerifyCode)
            throw new common_1.UnauthorizedException();
