...
let MemberGateway = (MemberGateway_1 = class MemberGateway {
    ...
    async botJoinGuild(guildOfMemberDto, botDto) {
        try {
            ...
            this.server.emit(
                `${botDto.botId}/${namespace_1.SocketNamespace.MEMBER}/${emit_1.MemberSocketEmit.JOIN}`,
                botMember
            )
            return botMember
        } catch (e) {
    ...
    async leaveGuild(memberId) {
        try {
            ...
            if (memberLeaveGuild.bot) {
                this.server.emit(
                    `${memberLeaveGuild.bot.botId}/${namespace_1.SocketNamespace.MEMBER}/${emit_1.MemberSocketEmit.LEAVE}`,
                    memberLeaveGuild
                )
            }
        } catch (e) {
