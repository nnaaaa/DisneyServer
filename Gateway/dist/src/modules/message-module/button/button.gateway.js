...
let ButtonGateway = (ButtonGateway_1 = class ButtonGateway {
    ...
    async create(actionOfButton, buttonOfButtonDto) {
        try {
            ...
            this.server.emit(
                `${actionOfButton.actionId}/${emit_1.ButtonSocketEmit.CREATE}`,
                savedButton
            )
        } catch (e) {
            this.logger.error(e)
    ...
    async update(updateButtonDto) {
        try {
            ...
            this.server.emit(
                `${emit_1.ButtonSocketEmit.UPDATE}/${updateButtonDto.buttonId}`,
                updatedButton
            )
    ...
    async click(buttonDto) {
        try {
            ...
            this.server.emit(
                `${button.action.actionId}/${namespace_1.SocketNamespace.BUTTON}/${emit_1.ButtonSocketEmit.CLICK}`,
                button
            )
        } catch (e) {
            this.logger.error(e)
...
ButtonGateway = ButtonGateway_1 = __decorate(
    [
        (0, websockets_1.WebSocketGateway)({
            cors: { origin: '*' },
            namespace: namespace_1.SocketNamespace.BUTTON,
        }),
