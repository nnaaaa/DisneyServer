...
let ReactGateway = ReactGateway_1 = class ReactGateway {
    ...
    async create(emojiOfReactDto, messageOfReactDto, authorOfReactDto) {
        try {
            const react = await this.reactService.reactToMessage(emojiOfReactDto, messageOfReactDto, authorOfReactDto);
            this.server.emit(`${messageOfReactDto.messageId}/${emit_1.ReactSocketEmit.CREATE}`, react);
        }
        catch (e) {
            this.logger.error(e);
...
__decorate([
    ...
    __metadata("design:paramtypes", [dtos_1.EmojiDto,
        dtos_1.MessageDto,
        dtos_1.MemberDto]),
...
ReactGateway = ReactGateway_1 = __decorate([
    (0, websockets_1.WebSocketGateway)({ cors: { origin: '*' }, namespace: 'react' }),
