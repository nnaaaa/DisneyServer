...
let UserService = class UserService {
    ...
    async changePassword(newPasswordDto, user) {
        ...
        if (
            newPasswordDto.constructor.name ===
            forgetPassword_dto_1.NewPasswordWithComparationDto.name
        ) {
            if (Bcrypt.compareSync(newPasswordDto.oldPassword, user.password)) {
                await updatePassword()
            } else throw new common_1.UnauthorizedException()
        } else if (
            newPasswordDto.constructor.name ===
            forgetPassword_dto_1.NewPassordWithSMSDto.name
        ) {
            await updatePassword()
        } else throw new common_1.BadRequestException()
    }
    ...
    async findBeFriend(userId, friendId) {
        if (userId === friendId) throw new common_1.BadRequestException()
        const user = await this.findOne({ userId })
        const friend = await this.findOne({ userId: friendId })
        if (!user || !friend) throw new common_1.NotFoundException()
    ...
    async acceptFriend(userId, friendId) {
        ...
        if (!beFriendInThePast) throw new common_1.NotFoundException()
...
UserService = __decorate(
    [
        ...
        __param(
            1,
            (0, typeorm_1.InjectRepository)(userBeFriend_entity_1.UserBeFriendEntity)
        ),
