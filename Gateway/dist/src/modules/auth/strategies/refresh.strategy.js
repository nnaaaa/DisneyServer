...
let RefreshTokenStrategy = class RefreshTokenStrategy extends (0,
passport_1.PassportStrategy)(passport_jwt_1.Strategy, 'jwt-refresh') {
    ...
    async validate(req, payload) {
        ...
        if (!refreshToken) throw new common_1.ForbiddenException()
        const user = await this.userService.findOne({ userId: payload.userId })
        if (!user) throw new common_1.NotFoundException()
        if (user.refreshToken !== refreshToken) throw new common_1.UnauthorizedException()
        return payload
    }
}
